...
const MenuItemCardComponent = ({ item: initialItemFromProps, categoryName }) => {
  const item = useContentfulLiveUpdates(initialItemFromProps);

  if (!item || !item.fields) {
    return null;
  }

  const { name, description, price, image, slug } = item.fields;
  const imageUrl = image?.fields?.file?.url;
  const imageAltText = image?.fields?.description || image?.fields?.title || name || 'Menu item';

  const detailPageLink = slug ? `/menu-item/${slug}` : '#';

  return (
    <RouterLink to={detailPageLink} style={{ textDecoration: 'none', display: 'block', height: '100%' }}>
      <Card
        hoverable
